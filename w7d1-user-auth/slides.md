...
## Queastions to ponder as you work on today's project
...
3. Why is it important that `session_token` be unique? Indexed?
4. Does `password_digest` need to be unique? Why or why not? Should it also be indexed?
...
8. Why do we only have one session (single resource)?
