-- CHALLENGE #1:
-- Select Walker's id
-- Walker is an instructor from San Francisco
SELECT
    id
FROM
    sf_instructors
...
-- CHALLENGE #2:
-- Select all of Walker's possessions using Walker's id
SELECT
    *
FROM
    possessions
...
FROM (
    ...
    GROUP BY
        type
    HAVING
        SUM(cost) > 200) AS pricey_possessions;
...
-- Visualize the sf_instructors table self-joining to itself
SELECT
